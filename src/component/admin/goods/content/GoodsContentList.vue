<template>
    <div  class="list">
        ...
        <el-table ref="multipleTable" :data="tableList" tooltip-effect="dark"
         style="width:100%" height="400"  align="left">
            <!-- 多选框列 -->
            ...
            <el-table-column width="100" label="状态">
                <!-- 在template里面，需要通过scope.row拿到每一行数据 -->
        ...
        <el-pagination @size-change="handleSizeChange" @current-change="handleSizeChange"
         :current-page="query.pageIndex" :page-size="query.pageSize" :page-sizes="[10,20,30,40]"
          layout="total,sizes,prev,pager,next,jumper" :total="totalcount"  align="left" >
        </el-pagination>
    </div>
</template>
<script>
export default {
  ...
  methods:{
    //   获取商品列表
    getTableList(){
        // get方法的第二个参数可以用来指定查询字符串，header头信息内容
        this.$http.get(this.$api.gsList,{ params:this.query})
        .then(rep=>{
            this.tableList=rep.data.message;
